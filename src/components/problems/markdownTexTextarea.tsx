    ...
    return (
        <label className={className ?? "block"}>
            ...
            <p className="mt-2 text-xs leading-5 text-slate-500">
                Markdown と TeX に対応しています。インライン数式は{" "}
                <code>$...$</code>、別行数式は <code>$$...$$</code> を使えます。
            </p>
            {helperText ? (
            ...
            <div className="mt-4 border border-slate-200 bg-slate-50">
                ...
                <div
                    className={`markdown-preview px-4 py-4 text-sm text-slate-800 ${previewMinHeightClassName}`}
                >
                    {deferredValue.trim().length > 0 ? (
                        <MarkdownTexContent content={deferredValue} />
                    ) : (
                        <p className="text-slate-400">
                            入力内容のプレビューがここに表示されます。
                        </p>
                    )}
                </div>
            </div>
